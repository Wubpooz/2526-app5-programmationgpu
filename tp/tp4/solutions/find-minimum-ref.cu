...
{
  __shared__ float buff[BLOCKSIZE];
  ...
  // Reduction au niveau d'un seul warp; pas besoin de synchronisation ni de if
  ...
  // Ecriture de la valeur finale, protection if car 32 threads actifs
  if (threadIdx.x == 0) dAmin[blockIdx.x] = buff[threadIdx.x];
}
